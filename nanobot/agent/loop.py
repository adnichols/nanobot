...
class AgentLoop:
    ...
    def _use_acp_for_session(self, session_key: str) -> bool:
        """Check if ACP should be used for a given session.

        ACP is used when:
        1. An ACP service is configured
        2. A default agent is specified in config

        Args:
            session_key: The session key (e.g., "telegram:12345")

        Returns:
            True if ACP should be used, False for local agent
        """
        if not self.acp_service:
            return False
        if not self.acp_default_agent:
            return False
        return True
    ...
    async def _process_message(
        ...
        if self._use_acp_for_session(key):
            ...
            if cmd in ("/new", "/help"):
                # Handle these commands locally even in ACP mode
                if cmd == "/help":
                    return OutboundMessage(
                        channel=msg.channel,
                        chat_id=msg.chat_id,
