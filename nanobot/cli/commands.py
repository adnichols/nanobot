...
# ACP service - will be initialized when needed
_acp_service = None


def _get_acp_service(config: Config):
    """Get or create the ACP service based on config."""
    global _acp_service
    ...
    if not config.acp.default_agent or not config.acp.agents:
        return None
...
def gateway(
    ...
    # Create agent with cron service and optional ACP
    agent = AgentLoop(
        bus=bus,
        provider=provider,
        workspace=config.workspace_path,
    ...
    channels = ChannelManager(config, bus, acp_service=acp_service)
